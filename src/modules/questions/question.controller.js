...
function message(error){
  ...
  if(String(error.message).startsWith('IMPORT_MISSING_COLUMN:')) return `File thiếu cột bắt buộc: ${error.message.split(':')[1]}.`;
...
async function downloadTemplate(req,res,next){try{const buffer=await service.template();res.setHeader('Content-Type','application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');res.setHeader('Content-Disposition','attachment; filename="question_import_template_v0.6.0.xlsx"');res.send(Buffer.from(buffer));}catch(e){next(e);}}
