...
async function editForm(req, res, next) {
  try {
    const { classItem, grades } = await service.getFormData(req.params.id);
...
async function update(req, res, next) {
  try {
    const result = await service.updateClass(req.params.id, req.body);
...
async function remove(req, res, next) {
  try {
    const ok = await service.deleteClass(req.params.id);
...
async function apiList(req, res, next) { try { res.json({ data: await service.getClasses() }); } catch (error) { next(error); } }
async function apiStudents(req, res, next) {
  try { const classItem = await service.getClassDetail(req.params.id); if (!classItem) return res.status(404).json({ message: 'Class not found' }); res.json({ data: classItem.students }); }
  catch (error) { next(error); }
}
