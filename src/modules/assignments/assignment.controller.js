...
async function index(req, res, next) {
  try {
    res.render('assignments/index', { title: 'Bài tập & chấm bài', ...(await service.list(req.query)), filters: req.query });
  } catch (error) { next(error); }
}

async function newForm(req, res, next) {
  try {
    res.render('assignments/new', { title: 'Giao bài tập', ...(await service.newForm(req.query)), error: null, values: req.query });
  } catch (error) { next(error); }
}

async function create(req, res, next) {
  try {
    const assignment = await service.create(req.body, req.session.user.id);
    res.redirect(`/assignments/${assignment.id}?created=1`);
  } catch (error) {
    if (Object.prototype.hasOwnProperty.call(MESSAGES, error.message)) {
      const data = await service.newForm({ classId: req.body.classId });
...
async function detail(req, res, next) {
  try {
    const assignment = await service.detail(req.params.id);
...
async function grade(req, res, next) {
  try {
    await service.grade(req.params.id, req.params.studentId, req.body);
...
async function apiList(req, res, next) {
  try { res.json({ data: (await service.list(req.query)).assignments }); } catch (error) { next(error); }
}
