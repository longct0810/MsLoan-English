...
async function newForm(req, res, next) {
  try {
    const { classes } = await service.getFormData();
...
async function create(req, res, next) {
  try {
    const result = await service.createStudent(req.body, req.session.user.id);
    if (result.errors) {
      const { classes } = await service.getFormData();
...
async function editForm(req, res, next) {
  try {
    const { student, classes } = await service.getFormData(req.params.id);
...
async function remove(req, res, next) {
  try {
    const ok = await service.deleteStudent(req.params.id);
...
async function apiList(req, res, next) {
  try { res.json({ data: await service.getStudents({ classId: req.query.classId }) }); }
  catch (error) { next(error); }
}
